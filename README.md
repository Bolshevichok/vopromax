...
Архитектура приложения (рис. 1) состоит из четырёх составляющих: [микросервис работы с Webhook API чат-бота MAX (Chatbot)](chatbot.md), [микросервис ответа на вопрос по документам (QA)](qa.md), база данных. Каждая составляющая вынесена в отдельный Docker-контейнер.
